/******/ (() => { // webpackBootstrap
/*!*****************************************!*\
  !*** ./src/brooklyn-ai-planner/view.js ***!
  \*****************************************/
/* eslint-env browser */
...
const initItineraryForm = () => {
  ...
  const chips = Array.from(form.querySelectorAll('.batp-form__chip input[type="checkbox"]'));

  // 0. Location Selection (Neighborhood + Geolocation)
  const neighborhoodSelect = document.getElementById('batp-neighborhood-select');
      ...
      if (option.value === 'current_location') {
        ...
        if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(position => {
            ...
            console.log('BATP Geolocation:', position.coords.latitude, position.coords.longitude);
          }, error => {
            console.error('Geolocation error:', error);
            if (locationStatus) {
              locationStatus.textContent = '❌ Location failed';
            }
            alert('Could not get your location. Please select a neighborhood.');
            neighborhoodSelect.value = 'Brooklyn Heights';
            setLocationFromOption(neighborhoodSelect.options[neighborhoodSelect.selectedIndex]);
          }, {
  ...
  chips.forEach(input => {
    input.addEventListener('change', () => {
      input.closest('.batp-form__chip').classList.toggle('is-selected', input.checked);
    });
  });
  ...
  const initMap = async (apiKey, locations = []) => {
    ...
    try {
      ...
      googleMap = new MapConstructor(mapContainer, {
        ...
        // Allow controls for better UX in full view
        zoomControl: true,
  ...
  const renderResults = (data, apiKey) => {
    ...
    itineraryItems.forEach((item, idx) => {
      ...
      if (lat && lng && lat !== 0 && lng !== 0) {
        ...
        console.warn(`BATP Map: Missing coordinates for item ${idx + 1}: ${item.title}`);
      }
    });
    ...
    const fullRouteUrl = multiStopDirectionsUrl !== '#' ? multiStopDirectionsUrl : buildMultiStopUrl(locations);
    ...
    const generateDescription = (details, itemDesc) => {
      ...
      const priceLevel = details.price_level ? '$'.repeat(details.price_level) : '';
      const types = details.types?.slice(0, 2).map(t => t.replace(/_/g, ' ')).join(', ') || '';
      if (rating) {
      ...
      return parts.length > 0 ? parts.join(' • ') : 'A local Brooklyn favorite.';
    };
    ...
    const getAvailableReplacements = () => {
      ...
      return allCandidates.filter(c => !currentSlugs.has(c.slug));
    };
    ...
    const renderList = items => {
      ...
      const headerHtml = `
			...
			`;
      listOutput.innerHTML = headerHtml + items.map((item, index) => {
        const candidate = candidateMap.get(item.slug);
        const details = candidate?.data || {};
        const website = details.website || '#';
        const phone = details.phone_number || details.phone || '';
        const address = details.address || '';
        const description = generateDescription(details, item.description);
        ...
        return `
				<div class="batp-card" data-slug="${item.slug}" data-access='${JSON.stringify(details.accessibility || [])}'>
					<div class="batp-card__actions">
						<button class="batp-card__replace" data-replace-slug="${item.slug}" data-replace-title="${item.title}" title="Replace with another venue">
							<span class="dashicons dashicons-update"></span>
						</button>
						<button class="batp-card__remove" data-remove-slug="${item.slug}" title="Remove from itinerary">×</button>
					</div>
					<div class="batp-card__header">
						<div>
							<span class="batp-card__status">${details.opening_hours?.open_now !== false ? 'Open Now' : 'Check Hours'}</span>
							<h3 class="batp-card__title">${index + 1}. ${item.title}</h3>
							${details.rating ? `<span class="batp-card__rating">⭐ ${details.rating}</span>` : ''}
						</div>
					</div>
					...
					<div class="batp-card__details">
						...
						${phone ? `<a href="tel:${phone.replace(/[^0-9+]/g, '')}" class="batp-card__link-row" data-event-action="phone_click" data-place-id="${placeId}"><span class="dashicons dashicons-phone"></span> ${phone}</a>` : ''}
					</div>
					...
					<div class="batp-card__footer">
						...
						<div class="batp-card__links">
							${website !== '#' ? `<a href="${website}" target="_blank" data-event-action="website_click" data-place-id="${placeId}" data-event-meta='${JSON.stringify({
          url: website
      ...
      listOutput.querySelectorAll('.batp-card__remove').forEach(btn => {
        btn.addEventListener('click', e => {
          e.preventDefault();
          const slugToRemove = btn.dataset.removeSlug;
          currentItems = currentItems.filter(i => i.slug !== slugToRemove);
      ...
      listOutput.querySelectorAll('.batp-card__replace').forEach(btn => {
        btn.addEventListener('click', e => {
          e.preventDefault();
    ...
    const replaceCurrentName = document.getElementById('batp-replace-current-name');
    const openReplaceModal = (slug, title) => {
      ...
      if (available.length === 0) {
        ...
        replaceList.innerHTML = available.map(c => {
          const rating = c.data?.rating ? `${c.data.rating}★` : '';
          const priceLevel = c.data?.price_level ? '$'.repeat(c.data.price_level) : '';
          return `
        ...
        replaceList.querySelectorAll('.batp-replace-option').forEach(opt => {
    ...
    const performReplacement = (oldSlug, newSlug) => {
      const newCandidate = allCandidates.find(c => c.slug === newSlug);
      if (!newCandidate) {
        return;
      }
    ...
    if (replaceModal) {
      replaceModal.querySelectorAll('[data-modal-close]').forEach(el => {
        el.onclick = e => {
          e.preventDefault();
          closeReplaceModal();
        };
    ...
    if (copyBtn) {
      copyBtn.onclick = () => {
        ...
        trackEvent('share_copy_link', null, {
          method: 'copy'
        });
      };
    }
    ...
    const buildShareText = () => {
      ...
      const venueList = itineraryItems.map((item, i) => `${i + 1}. ${item.title}`).join('\n');
    ...
    if (pdfBtn) {
      pdfBtn.onclick = () => {
        ...
        if (!printWindow) {
          alert('Please allow popups for this site to download your itinerary.');
          return;
        }
    ...
    if (calBtn) {
      calBtn.onclick = () => {
        ...
        const venues = itineraryItems.map(v => v.title).join(', ');
        const description = `Brooklyn Adventure:\\n\\n${itineraryItems.map((v, i) => `${i + 1}. ${v.title}${v.address ? ' - ' + v.address : ''}`).join('\\n')}`;
        const icsContent = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Brooklyn AI Trip Planner//EN', 'BEGIN:VEVENT', `UID:${Date.now()}@visitbrooklynnyc`, `DTSTAMP:${formatDate(now)}`, `DTSTART:${formatDate(tomorrow)}`, `DTEND:${formatDate(new Date(tomorrow.getTime() + 4 * 60 * 60 * 1000))}`, `SUMMARY:Brooklyn Adventure`, `DESCRIPTION:${description}`, `LOCATION:${venues}`, 'END:VEVENT', 'END:VCALENDAR'].join('\r\n');
    ...
    if (emailBtn) {
      emailBtn.onclick = () => {
        const subject = encodeURIComponent('Check out my Brooklyn itinerary!');
    ...
    const shareText = encodeURIComponent(`I just planned a Brooklyn adventure with ${itineraryItems.length} stops!`);
    ...
    if (xBtn) {
      xBtn.onclick = () => {
        window.open(`https://twitter.com/intent/tweet?url=${shareUrl}&text=${shareText}`, '_blank', 'width=600,height=400');
        trackEvent('share_social', null, {
          platform: 'x'
        });
      };
    ...
    if (igBtn) {
      igBtn.onclick = () => {
        ...
        navigator.clipboard.writeText(text).then(() => {
          alert('Itinerary copied to clipboard! Opening Instagram - paste in your story or post.');
    ...
    if (ttBtn) {
      ttBtn.onclick = () => {
        const text = buildShareText();
        navigator.clipboard.writeText(text).then(() => {
          alert('Itinerary copied to clipboard! Opening TikTok - paste in your video description.');
    ...
    try {
      const response = await fetch(eventsUrl, {
        ...
        body: JSON.stringify({
          action_type: action,
          place_id: placeId,
          // Use place_id for Google Places
          metadata: finalMeta,
          nonce
        })
      });
      console.log('BATP trackEvent response:', response.status, response.ok);
    } catch (err) {
  ...
  if (listOutput) {
    listOutput.addEventListener('click', e => {
      ...
      if (target) {
        ...
        const meta = target.dataset.eventMeta ? JSON.parse(target.dataset.eventMeta) : {};
